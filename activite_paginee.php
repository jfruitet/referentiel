<?php  // $Id: activite_paginee.php,v 1.0 2014/03/25 00:00:00 jfruitet Exp $
///////////////////////////////////////////////////////////////////////////
//                                                                       //
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
...
///////////////////////////////////////////////////////////////////////////
    ...
    include('print_lib_activite_paginee.php');	// AFFICHAGES pagines des  acivites
...
//*********************************************************************
    ...
    $url = new moodle_url('/mod/referentiel/activite_paginee.php');
	...
	elseif ($id) {
		...
		print_error(get_string('erreurscript','referentiel','Erreur01 : activite_paginee.php'), 'referentiel');
	}
	...
	// DEBUG
	//echo "<br /> DEBUG :: activite_paginee.php :: 68 :: MODE : $mode\n";
    if ($mode=='activitespaginees'){
        $modeaff=1;
	}
	else{
        $modeaff=0;
            ...
            switch ($mode) {
                ...
                case "updateactivity":
                    ...
                    if (isset($form->delete) && ($form->delete==get_string('delete'))){
                        ...
                        if (!$return) {
    	         	      	print_error("Could not update activity $form->activite_id of the referentiel", "activite_paginee.php?d=$referentiel->id");
                        }
                        if (is_string($return)) {
                            print_error($return, 'error', "activite_paginee.php?d=$referentiel->id&amp;userid=$form->userid");
                        }
                    ...
                    //DEBUG
					//echo "<br />activite_paginee.php :: 425 \n";
					//print_object($form);
					//
					//exit;
                        $return = $updatefunction($form);
                        if (!$return) {
                            print_error("Could not update activity $form->id of the referentiel", 'error', "activite_paginee.php?d=$referentiel->id");
                        }
                        if (is_string($return)) {
    	        		    print_error($return, "activite_paginee.php?d=$referentiel->id");
                        }
	...
	if ($records_id_users){
	    ...
	    if (!empty($referentiel->ref_referentiel)){
			...
			if ($totalRecords >0){
		    	...
		    	$sql = str_replace("\n","",$sql);    // hack
	// JavaScript Document
	//echo "<br />DEBUG :: 861 :: ".htmlentities($sql)."\n";
	//echo "<br />DEBUG :: 862 :: ".urlencode($sql)."\n";
	//exit;
    //$sql = str_replace('>','&gt;',$sql);    // hack
    //$sql = str_replace('<','&lt;',$sql);    // hack
...
/*
    if (has_capability('mod/referentiel:managetemplates', $context)) {
        if (!record_exists('referentiel_activite','ref_referentiel',$referentiel->id)) {      // Brand new referentielbase!
            redirect($CFG->wwwroot.'/mod/referentiel/activite_paginee.php?d='.$referentiel->id);  // Redirect to field entry
        }
    }
*/
...
// AFFICHAGE DE LA PAGE Moodle 2
    ...
    // JF 2011/11/29 - Affiche destinataires en cas de notification
    if ($mailnow && $activite_id) { // id 	activite
        $destinataires=referentiel_get_referents_notification(get_record('referentiel_activite', 'id', $activite_id));
        if ($destinataires->nbdestinataires){
            echo '<div align="center"> <a class="overlib" href="javascript:void(0);" onmouseover="return overlib(\''.$destinataires->liste_destinataires.'\', WIDTH, 500, STICKY, MOUSEOFF, VAUTO, FGCOLOR, \'#DDEEFF\', CAPTION, \''.get_string('referents', 'referentiel').'\');" onmouseout="return nd();">'.get_string('destinataires_notification', 'referentiel').'</a></div>'."\n";
        }
    }
...
<img src="'.$OUTPUT->pix_url('ajax-loader','referentiel').'" onload="'.$onclick.'">
    ...
    // echo '<button id="clickme2"  onclick="'.$onclick2.'">Tester</button>'."\n";
    echo $OUTPUT->footer();
    die();
    print_footer($course);
