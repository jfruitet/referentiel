...
// AFFICHAGE DE LA PAGE
    ...
    elseif ($mform->no_submit_button_pressed()) {
            ...
            exit;
            referentiel_add_or_update_activity($mform, $referentiel->id, $context);
            die();
    }
    else {
...
//------------------
function referentiel_add_or_update_activity($mform, $instanceid, $context){
...
global $CFG, $USER, $DB, $OUTPUT;
    ...
    if ($formdata = $mform->get_data()) {
        // DEBUG
        echo "<br />DEBUG :: lib.php :: 352 :: referentiel_ad_or_update_activity\n";
        print_object($formdata->code_item);
/*
        $activite = new object();
        $activite->id=0;
...
*/
        ...
        $retour_url=new moodle_url('/mod/referentiel/activite.php', array('d'=>$instanceid, 'userid'=>$formdata->userid, 'activite_id'=>$formdata->activite_id, 'mailnow' => $formdata->mailnow, 'mode' => 'listactivityall', 'select_acc' => $formdata->select_acc));

        if (!empty($formdata->activite_id)){
            $returnid=referentiel_update_activity($formdata);
            if (!$returnid) {
                print_error("Could not update activity $formdata->id of the referentiel", 'error', "activite.php?d=$instanceid");
            }
            if (is_string($returnid)) {
                print_error($returnid, "activite.php?d=$referentiel_id");
            }
            add_to_log($formdata->courseid, "referentiel", "update","mise a jour activite $formdata->activite_id", $instance, "");
            // depot de document ?
            ...
            $returnid=referentiel_add_activity($formdata);
		    if (!$returnid) {
                print_error("Could not add a new activity to the referentiel", 'error', "activite.php?d=$instanceid");
			}
            if (is_string($returnid)) {
                print_error($returnid, 'error', "activite.php?d=$instanceid&amp;select_acc=$formdata->select_acc&amp;userid=$formdata->userid");
			}
            // depot de document ?
			if (isset($formdata->depot_document) && ($formdata->depot_document==get_string('yes'))){
                // APPELER le script
                if ($returnid){
                    redirect($CFG->wwwroot.'/mod/referentiel/upload_moodle2.php?d='.$instanceid
                            .'&amp;userid='.$formdata->userid
                            .'&amp;activite_id='.$returnid
                            .'&amp;select_acc='.$formdata->select_acc
                            .'&amp;mailnow='.$formdata->mailnow
                            .'&amp;document_id=0&amp;mode=adddocument&amp;filtre_auteur='.$formdata->filtre_auteur.'&amp;filtre_validation='.$formdata->filtre_validation.'&amp;filtre_referent='.$formdata->filtre_referent.'&amp;filtre_date_modif='.$formdata->filtre_date_modif.'&amp;filtre_date_modif_student='.$formdata->filtre_date_modif_student
                            .'&amp;old_mode=listactivityall&amp;sesskey='.sesskey());
                    exit;
                }
            }
            add_to_log($formdata->courseid, "referentiel", "add", "creation activite $returnid ", $instanceid, "");

            $mode ='listactivityall';
			if (has_capability('mod/referentiel:managecertif', $context)){
                $retour_url = "$CFG->wwwroot/mod/referentiel/activite.php?d=$instanceid&amp;select_acc=$formdata->select_acc&amp;userid=$formdata->userid&mailnow=$formdata->mailnow&amp;mode=$formdata->mode&amp;filtre_auteur=$formdata->filtre_auteur&amp;filtre_validation=$formdata->filtre_validation&amp;filtre_referent=$formdata->filtre_referent&amp;filtre_date_modif=$formdata->filtre_date_modif&amp;filtre_date_modif_student=$formdata->filtre_date_modif_student";
			}
			else{
                if ($formdata->mailnow){
                    $retour_url = "$CFG->wwwroot/mod/referentiel/activite.php?d=$instanceid&amp;select_acc=$formdata->select_acc&amp;userid=$formdata->userid&activite_id=$return&mailnow=$formdata->mailnow&amp;mode=$formdata->mode&amp;filtre_auteur=$formdata->filtre_auteur&amp;filtre_validation=$formdata->filtre_validation&amp;filtre_referent=$formdata->filtre_referent&amp;filtre_date_modif=$formdata->filtre_date_modif&amp;filtre_date_modif_student=$formdata->filtre_date_modif_student";
                }
                else{
                    $retour_url = "$CFG->wwwroot/mod/referentiel/activite.php?d=$instanceid&amp;select_acc=$formdata->select_acc&amp;userid=$formdata->userid&amp;mode=$formdata->mode&amp;filtre_auteur=$formdata->filtre_auteur&amp;filtre_validation=$formdata->filtre_validation&amp;filtre_referent=$formdata->filtre_referent&amp;filtre_date_modif=$formdata->filtre_date_modif&amp;filtre_date_modif_student=$formdata->filtre_date_modif_student";
                }
			}
        }
        if (!empty($retour_url)){
